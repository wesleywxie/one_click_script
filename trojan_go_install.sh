    ...
    cat > ${configTrojanBasePath}/server.json <<-EOF
{
    "run_type": "server",
    "local_addr": "0.0.0.0",
    "local_port": $configTrojanPort,
    "remote_addr": "127.0.0.1",
    "remote_port": 80,
    "password": [
        "${configTrojanPwdInput}"
    ],
    "log_level": 5,
    "ssl": {
        "verify": true,
...
EOF
	...
	red "    Trojan 访问日志运行 journalctl -u trojan${promptInfoTrojanName}.service 查看 !"
	...
	yellow "服务器地址: ${configSSLDomain}  端口: $configTrojanPort"
	yellow "您指定的密码: ${configTrojanPwdInput}"

    if [[ ${isSupportWebsocket} == "true" ]]; then
